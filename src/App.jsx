...
const pathToPage = {
  ...
  '/terms': 'terms', '/help': 'help', '/careers': 'careers', '/saved': 'saved',
}
...
function App() {
  ...
  const [saved, setSaved] = useState(() => {
    try { return JSON.parse(localStorage.getItem('flavor-fusion-saved')) || [] } catch { return [] }
  })
  ...
  const toggleSave = (name) => setSaved((current) => current.includes(name) ? current.filter((item) => item !== name) : [...current, name])
  ...
  if (page === 'saved') return <Saved saved={saved} onToggleSave={toggleSave} onSetSaved={setSaved} navigate={navigate} />
  ...
  return <Explore selected={selected} onSelected={setSelected} saved={saved} onToggleSave={toggleSave} navigate={navigate} />
}
